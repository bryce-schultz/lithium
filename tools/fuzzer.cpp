...
public:
    ...
    string mutateValidCode()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                string toInsert = (rng() % 2) ? keywords[rng() % keywords.size()] : operators[rng() % operators.size()];
    ...
    string generateTestCase()
    {
        ...
        case 1:
            return generateMalformedSyntax();
        case 2:
            return generateEdgeCase();
        case 3:
            return mutateValidCode();
        default:
        {
    ...
    void runFuzzingCampaign(int numTests = 1000, const string &logFile = "fuzzer_results.log")
    {
        ...
        for (int i = 0; i < numTests; i++)
        {
            ...
            if (interesting)
            {
                ...
                if (result.timeout)
                    timeouts++;
                if (result.exitCode != 0 && !result.crashed)
                    errors++;
            }
        }
